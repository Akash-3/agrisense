...
def add_native_toc(doc):
    ...
    r1._r.append(fldChar3)

def generate_upgraded_report(output_path, activity_flow_img, drone_img, ground_img):
    doc = Document()
    
    # Page setup (1 inch top/bottom, 1.25 left, 1.0 right)
    ...
    def add_doc_title(text):
        ...
        run.font.size = Pt(22)
        run.font.bold = True
        run.font.color.rgb = RGBColor(0x1E, 0x3A, 0x8A) # BOLD BLUE LARGE
        return p
    ...
    def add_ch_heading(title):
        ...
        run = p.add_run(title)
        run.font.name = 'Times New Roman'
        run.font.size = Pt(16) # BOLD BLUE 16PT
        run.font.bold = True
    ...
    def add_sec_heading(title):
        ...
        run = p.add_run(title)
        run.font.name = 'Times New Roman'
        run.font.size = Pt(13.5) # BOLD BLUE 13.5PT
        run.font.bold = True
    ...
    def add_sub_heading(title):
        ...
        run = p.add_run(title)
        run.font.name = 'Times New Roman'
        run.font.size = Pt(12) # BOLD BLUE 12PT
        run.font.bold = True
    ...
    def add_figure(img_path, caption_text, width_inches=4.8):
    ...
    # ==================== PAGE 1: COVER PAGE ====================
    add_doc_title("AGRISENSE: A LOW-COST IOT DRONE PLATFORM FEATURING DUAL-STREAM MULTI-MODAL SPECTRAL-SPATIAL AI FOR EARLY ASYMPTOMATIC CROP DISEASE DIAGNOSIS AND SOIL HEALTH MONITORING")
    add_doc_subtitle("A Final-Year Project Report Submitted in Partial Fulfillment of the Requirements for T.Y. B.Sc. IT Semester-V")
    ...
    contents = [
        ...
        ("   2.c Gantt Chart & Activity Flow Diagram", "16"),
    ...
    # ==================== PAGE 13: 2.b Timeline & Gantt Chart ====================
    add_sec_heading("2.b Timeline for each task (Gantt Chart)")
    add_p("The 12-week project implementation schedule allocation is represented below as a native Gantt chart table featuring red execution bars:")

    # NATIVE GANTT CHART TABLE
    table_gantt = doc.add_table(rows=15, cols=13)
    table_gantt.alignment = WD_TABLE_ALIGNMENT.CENTER
    
    headers_gantt = ["Activity / Task", "W1", "W2", "W3", "W4", "W5", "W6", "W7", "W8", "W9", "W10", "W11", "W12"]
    for i, title in enumerate(headers_gantt):
        c = table_gantt.rows[0].cells[i]
        c.text = title
        p = c.paragraphs[0]
    ...
    for row_idx, (act_name, active_weeks) in enumerate(activities_data, start=1):
        row_cells = table_gantt.rows[row_idx].cells
        ...
        for w in range(1, 13):
            ...
            if w in active_weeks:
                # Red bar execution timeline fill (#DC2626)
    ...
    p_gantt_note = doc.add_paragraph()
    p_gantt_note.paragraph_format.space_before = Pt(4)
    p_gantt_note.paragraph_format.space_after = Pt(12)
    r_gn = p_gantt_note.add_run("Table 2.1: AgriSense 12-Week Project Implementation Gantt Chart (Red Bar Execution Timeline).")
    r_gn.font.size = Pt(9.5)
    r_gn.font.italic = True
    r_gn.font.color.rgb = RGBColor(0x47, 0x55, 0x69)

    add_sec_heading("2.c System Activity Flow Diagram")
    add_p("The runtime system activity sequence follows a non-blocking operational workflow as illustrated in the Activity Flow Diagram below:")

    add_figure(activity_flow_img, "Figure 2.1: AgriSense System Activity Flowchart (Sample Sensors -> Serialize JSON -> HTTP POST to API -> Validate via Pydantic -> Run MM-SSNet CHI -> Persist to PostgreSQL -> Backend -> App or Website).", width_inches=3.6)
    ...
    add_figure(drone_img, "Figure 3.1: AgriSense Aerial Drone Sensor Payload showing ESP32 DevKit V1, Adafruit AS7341 10-Channel Multi-Spectral Sensor, DHT22 Microclimate Sensor, and MQ-2 Gas/Smoke Sensor.", width_inches=5.4)
    ...
    add_figure(ground_img, "Figure 3.2: AgriSense Ground Node Soil Monitoring IoT Unit showing overview, internal circuit wiring (ESP32 DevKit V1 + TP4056 + 18650 Battery), specifications, and field deployment setup.", width_inches=5.6)
    ...
    print(f"Successfully generated Upgraded Report Word Document at: {output_path}")

if __name__ == "__main__":
    onedrive_doc = r"C:\Users\tempm\OneDrive\Documents\AgriSense Report.docx"
    docs_dir = r"C:\Users\tempm\.gemini\antigravity\scratch\agrisense\docs"
    scratch_doc = os.path.join(docs_dir, "AgriSense_Final_Report.docx")
    
    activity_flow_img = r"C:\Users\tempm\.gemini\antigravity\brain\674aae1c-2705-4eac-b2ee-f2be6b55fad5\.user_uploaded\media_1786102931388.jpg"
    drone_img = os.path.join(docs_dir, "agrisense_drone_payload.jpg")
    ground_img = os.path.join(docs_dir, "agrisense_ground_node.jpg")

    generate_upgraded_report(onedrive_doc, activity_flow_img, drone_img, ground_img)
    generate_upgraded_report(scratch_doc, activity_flow_img, drone_img, ground_img)
