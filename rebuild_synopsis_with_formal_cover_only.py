...
def generate_synopsis_formal_cover(docx_path, md_path):
    print(f"Rebuilding AgriSense Synopsis with Formal Cover Page as Page 1 at {docx_path}...")
    doc = Document()
    
    # Configure Section 1 (Cover Page - No Footer Page Numbering)
    sec_cover = doc.sections[0]
    sec_cover.top_margin = Inches(1.25)
    sec_cover.bottom_margin = Inches(1.25)
    sec_cover.left_margin = Inches(1.25)
    sec_cover.right_margin = Inches(1.25)
    ...
    r_t = p_title.add_run("AgriSense: A Low-Cost IoT Drone Platform for Early Crop Disease Diagnosis and Soil Health Monitoring Using Dual-Stream Spectral-Spatial AI")
    r_t.font.name = 'Times New Roman'
    r_t.font.size = Pt(22)
    r_t.font.bold = True
    r_t.font.color.rgb = RGBColor(0x1E, 0x3A, 0x8A) # Deep Blue

    # Subtitle
    p_sub = doc.add_paragraph()
    p_sub.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_sub.paragraph_format.space_before = Pt(12)
    p_sub.paragraph_format.space_after = Pt(64)
    r_sub = p_sub.add_run("A Project Synopsis Submitted in Partial Fulfillment of the Requirements for T.Y. B.Sc. IT Semester-V")
    ...
    p_sub_by.paragraph_format.space_after = Pt(4)
    ...
    for name, roll in authors:
        p_auth = doc.add_paragraph()
        p_auth.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p_auth.paragraph_format.space_after = Pt(2)
        r_a = p_auth.add_run(f"{name}, {roll}")
        r_a.font.name = 'Times New Roman'
        r_a.font.size = Pt(12)

    # Under Guidance Of
    p_g_label = doc.add_paragraph()
    p_g_label.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_g_label.paragraph_format.space_before = Pt(28)
    p_g_label.paragraph_format.space_after = Pt(4)
    r_gl = p_g_label.add_run("Under the Guidance of:")
    r_gl.font.name = 'Times New Roman'
    r_gl.font.size = Pt(12)

    p_guide = doc.add_paragraph()
    p_guide.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_guide.paragraph_format.space_after = Pt(48)
    r_gn = p_guide.add_run("Jinal Gujar")
    r_gn.font.name = 'Times New Roman'
    r_gn.font.size = Pt(12)

    # Department & Year Footer Block
    p_dept = doc.add_paragraph()
    p_dept.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_dept.paragraph_format.space_after = Pt(4)
    r_d = p_dept.add_run("DEPARTMENT OF INFORMATION TECHNOLOGY")
    r_d.font.name = 'Times New Roman'
    r_d.font.size = Pt(12)
    r_d.font.color.rgb = RGBColor(0x1E, 0x3A, 0x8A)

    p_yr = doc.add_paragraph()
    p_yr.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_yr.paragraph_format.space_after = Pt(0)
    r_y = p_yr.add_run("ACADEMIC YEAR 2025 – 2026")
    r_y.font.name = 'Times New Roman'
    r_y.font.size = Pt(12)
    r_y.font.color.rgb = RGBColor(0x1E, 0x3A, 0x8A)

    # PAGE BREAK AFTER COVER PAGE
    doc.add_page_break()

    # Section 2 for Body (with Page X of Y footer page numbers)
    sec_body = doc.add_section()
    sec_body.top_margin = Inches(1.0)
    sec_body.bottom_margin = Inches(1.0)
    sec_body.left_margin = Inches(1.0)
    sec_body.right_margin = Inches(1.0)
    add_page_number_to_footer(sec_body)
    ...
    for r_idx, (num, name, desc) in enumerate(modules_data, start=1):
        ...
        for c_idx in range(3):
            ...
            if cell_p.runs:
                cell_p.runs[0].font.name = 'Times New Roman'
    ...
    doc.save(docx_path)
    print(f"Successfully generated AgriSense Synopsis with Formal Cover Page at: {docx_path}")

    # EXPORT MARKDOWN
    doc_read = Document(docx_path)
    md_lines = []
    
    md_lines.append("# AGRISENSE: A LOW-COST IOT DRONE PLATFORM FOR EARLY CROP DISEASE DIAGNOSIS AND SOIL HEALTH MONITORING USING DUAL-STREAM SPECTRAL-SPATIAL AI")
    md_lines.append("### A Project Synopsis Submitted in Partial Fulfillment of the Requirements for T.Y. B.Sc. IT Semester-V\n")
    md_lines.append("**Submitted By:** Group – G580")
    md_lines.append("* Kavya Bhandary, 261796")
    md_lines.append("* Rehaan Shaikh, 261831")
    md_lines.append("* Aditya Surve, 261844\n")
    md_lines.append("**Under the Guidance of:** Jinal Gujar\n")
    md_lines.append("**DEPARTMENT OF INFORMATION TECHNOLOGY**")
    md_lines.append("**ACADEMIC YEAR 2025 – 2026**\n")
    md_lines.append("---\n")

    for p in doc_read.paragraphs:
        t = p.text.strip()
        if not t or "AgriSense: A Low-Cost" in t or "A Project Synopsis Submitted" in t or "Submitted By:" in t or "Group – G580" in t or "Under the Guidance of:" in t or "DEPARTMENT OF INFORMATION" in t or "ACADEMIC YEAR" in t:
            continue
