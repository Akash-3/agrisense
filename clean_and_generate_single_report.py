...
def clean_old_files(docs_dir, master_name):
    print("Cleaning up old report files in docs/...")
    pattern_docx = os.path.join(docs_dir, "*.docx")
    pattern_md = os.path.join(docs_dir, "*.md")
    
    files_to_remove = glob.glob(pattern_docx) + glob.glob(pattern_md)
    for f in files_to_remove:
        basename = os.path.basename(f)
        if basename.startswith("~$"):
            continue
        if basename != master_name + ".docx" and basename != master_name + ".md":
            try:
                os.remove(f)
...
def generate_single_master_report(output_docx_path):
    doc = Document()
    ...
    doc.add_page_break() # PAGE 25: INTEGRATED BIBLIOGRAPHY, CREDITS & IEEE REFERENCES (STANDALONE UN-NUMBERED SECTION)
    ...
    print(f"Successfully generated Single Master Report Word Document at: {output_docx_path}")

if __name__ == "__main__":
    docs_directory = r"C:\Users\tempm\.gemini\antigravity\scratch\agrisense\docs"
    master_name = "AgriSense_Final_Report"
    
    clean_old_files(docs_directory, master_name)
    
    master_docx_path = os.path.join(docs_directory, master_name + ".docx")
    generate_single_master_report(master_docx_path)
