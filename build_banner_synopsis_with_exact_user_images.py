...
def generate_synopsis_with_exact_user_images(docx_path, md_path):
    print(f"Building AgriSense Project Synopsis with EXACT user-uploaded images at {docx_path}...")
    doc = Document()
    ...
    def add_image_figure(img_path, caption_text, width_inches=5.5):
    ...
    # ==================== 5. PROPOSED MODULES & HARDWARE DIAGRAMS ====================
    ...
    # USER IMAGE 1: DRONE PAYLOAD (media_1788249540353.png)
    add_sub_heading("Module 1 Detail: Aerial Drone Multi-Spectral Sensor Payload")
    add_p("The aerial drone sensing payload mounts directly onto the quadcopter belly, integrating an ESP32 DevKit V1 board, Adafruit AS7341 10-channel optical breakout board, DHT22 microclimate sensor, MQ-2 gas sensor, and LiPo power supply into a feather-light 82-gram package:")
    
    img_drone_user = r"C:\Users\tempm\.gemini\antigravity\brain\674aae1c-2705-4eac-b2ee-f2be6b55fad5\.user_uploaded\media_1788249540353.png"
    add_image_figure(img_drone_user, "Figure 5.1: Custom Quadcopter Drone Payload with ESP32 DevKit, Adafruit AS7341 10-Channel Optical Sensor, and DHT22 Microclimate Probe Mounted Underneath", width_inches=5.5)

    # USER IMAGE 2: GROUND NODE INFOGRAPHIC (media_1788249529387.png)
    add_sub_heading("Module 2 Detail: Terrestrial Ground Soil Hydration Node")
    add_p("The stationary ground soil module isolates heavy edaphic probes from the flying drone. The architectural breakdown below illustrates the field enclosure, internal hardware wiring, circuit schematic, technical specifications, operating workflow, and field installation:")

    img_ground_user = r"C:\Users\tempm\.gemini\antigravity\brain\674aae1c-2705-4eac-b2ee-f2be6b55fad5\.user_uploaded\media_1788249529387.png"
    add_image_figure(img_ground_user, "Figure 5.2: AgriSense Ground Node Architectural Breakdown (Overview, Internal Component View, Circuit Diagram, Specifications Table, Operating Workflow, and Field Deployment)", width_inches=5.8)
    ...
    doc.save(docx_path)
    print(f"Successfully generated AgriSense Synopsis with EXACT user-uploaded images at: {docx_path}")
    ...
    md_lines.append(f"![Aerial Drone Sensor Payload](file:///{img_drone_user.replace(os.sep, '/')})")
    
    md_lines.append("\n### Figure 5.2: AgriSense Ground Node Architectural Infographic")
    md_lines.append(f"![Ground Node Architectural Breakdown](file:///{img_ground_user.replace(os.sep, '/')})")
...
if __name__ == "__main__":
    ...
    generate_synopsis_with_exact_user_images(syn_docx_path, syn_md_path)
