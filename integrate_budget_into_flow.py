...
def integrate_budget_into_flow(docx_path):
    print(f"Seamlessly integrating Budget analysis into the natural chapter flow of {docx_path}...")
    doc = Document(docx_path)
    
    # 1. Remove floating '7. Budget & Cost Estimation Analysis' if present
    p_to_remove = []
    for p in doc.paragraphs:
        t = p.text.strip()
        if t.startswith("7. Budget & Cost Estimation") or t.startswith("4.b Economic Viability") or t.startswith("Budget & Cost Estimation Overview:") or "Zero Software Licensing Fees:" in t or "Affordable Hobby Hardware:" in t or "Commercial Cost Barrier:" in t or t.startswith("Financial Summary: Total hardware cost"):
    ...
    # 2. Update 1.e Economic Feasibility paragraph text to reference Indian Rupees ₹50,000 - ₹1,00,000
    for p in doc.paragraphs:
        t = p.text.strip()
        if t.startswith("2. Economic Feasibility:"):
            p.text = "2. Economic Feasibility: Commercial precision agriculture survey drones cost over ₹50,000 to ₹1,00,000, making them unaffordable for smallholder farmers. AgriSense constructs a complete dual-node IoT system for a total hardware budget of ₹5,930 by leveraging off-the-shelf ESP32 boards (₹1,000), AS7341 optical sensors (₹2,500), soil/climate probes (₹730), and hobby drone frames (₹1,700), combined with 100% free open-source software (FastAPI, React, PyTorch). (See Section 4.b for itemized breakdown)."

    # 3. Locate main Chapter 5 heading '5. CONCLUSION & FUTURE SCOPE'
    target_idx = None
    for i, p in enumerate(doc.paragraphs):
        t = p.text.strip()
        if i > 120 and ("5. CONCLUSION & FUTURE SCOPE" in t or t == "5. CONCLUSION & FUTURE SCOPE"):
            target_idx = i
            break
    ...
    # Insert 4.b Economic Viability & Cost Estimation Analysis BEFORE Chapter 5
    p_sec = target_p.insert_paragraph_before()
    p_sec.paragraph_format.space_before = Pt(18)
    p_sec.paragraph_format.space_after = Pt(8)
    r_sec = p_sec.add_run("4.b Economic Viability & Budget Cost Estimation Analysis")
    r_sec.font.name = 'Times New Roman'
    r_sec.font.size = Pt(13.5) # SECTION HEADING 13.5PT BOLD BLUE
    r_sec.font.bold = True
    r_sec.font.color.rgb = RGBColor(0x1E, 0x40, 0xAF)
    ...
    bullets = [
        ...
        ("• Commercial Cost Barrier:", "Industrial farm survey drones cost over ₹50,000 to ₹1,00,000, placing them out of reach for small family farms and educational institutions.")
    ]
    ...
    doc.save(docx_path)
    print(f"Successfully integrated Budget analysis into {docx_path}!")
...
if __name__ == "__main__":
    ...
    integrate_budget_into_flow(onedrive_path)
    integrate_budget_into_flow(scratch_path)
