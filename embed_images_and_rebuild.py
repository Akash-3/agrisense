...
def copy_images_to_docs(docs_dir, brain_dir):
    # 1. Copy generated drone image
    drone_src = os.path.join(brain_dir, "agrisense_drone_payload_1786084692326.jpg")
    drone_dst = os.path.join(docs_dir, "agrisense_drone_payload.jpg")
    if os.path.exists(drone_src):
        shutil.copy(drone_src, drone_dst)
        print(f"Copied Drone image to: {drone_dst}")
    else:
        # Search for any agrisense_drone_payload image in brain_dir
        matches = glob.glob(os.path.join(brain_dir, "agrisense_drone_payload*.jpg"))
        if matches:
            shutil.copy(matches[0], drone_dst)
            print(f"Copied Drone image ({matches[0]}) to: {drone_dst}")

    # 2. Copy user uploaded ground node image
    user_uploaded_dir = os.path.join(brain_dir, ".user_uploaded")
    ground_src = os.path.join(user_uploaded_dir, "media_1786038773771.jpg")
    ground_dst = os.path.join(docs_dir, "agrisense_ground_node.jpg")
    if os.path.exists(ground_src):
        shutil.copy(ground_src, ground_dst)
        print(f"Copied Ground Node image to: {ground_dst}")
    else:
        matches = glob.glob(os.path.join(user_uploaded_dir, "*.jpg")) + glob.glob(os.path.join(user_uploaded_dir, "*.png"))
        if matches:
            shutil.copy(matches[0], ground_dst)
            print(f"Copied Ground Node image ({matches[0]}) to: {ground_dst}")

    return drone_dst, ground_dst

def update_master_report_with_images(master_docx_path, drone_img, ground_img):
    print("Rebuilding AgriSense_Final_Report.docx with embedded figures...")
    doc = Document()
    ...
    add_figure(drone_img, "Figure 3.1: AgriSense Aerial Drone Sensor Payload showing ESP32 DevKit V1, Adafruit AS7341 10-Channel Spectral Sensor, DHT22 Microclimate Sensor, and MQ-2 Hazard Sensor.", width_inches=5.6)
    ...
    add_figure(ground_img, "Figure 3.2: AgriSense Ground Node Soil Monitoring IoT Unit showing overview, internal circuit wiring (ESP32 DevKit V1 + TP4056 + 18650 Battery), circuit diagram, specifications, and field deployment setup.", width_inches=5.8)
    ...
    print(f"Successfully saved Master Report with images at: {master_docx_path}")

def update_synopsis_with_images(synopsis_docx_path, drone_img, ground_img):
    print("Rebuilding AgriSense_Project_Synopsis.docx with embedded figures...")
    doc = Document()
    ...
    def add_figure(img_path, caption_text, width_inches=5.6):
        if os.path.exists(img_path):
            p = doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            p.paragraph_format.space_before = Pt(10)
    ...
    add_heading("5. System Architecture & Component Specifications")
    add_p("The complete architectural model for AgriSense was constructed using Draw.io (diagrams.net). Below are the hardware payload photograph and ground node layout diagram:")

    add_figure(drone_img, "Figure 5.1: AgriSense Aerial Drone Payload showing ESP32 DevKit V1, Adafruit AS7341 10-Channel Multi-Spectral Sensor, DHT22 Microclimate Sensor, and MQ-2 Gas/Smoke Sensor.", width_inches=5.4)
    add_figure(ground_img, "Figure 5.2: AgriSense Ground Node Soil Monitoring IoT Unit showing overview, internal circuit wiring (ESP32 + TP4056 + 18650 Battery), specifications, and field deployment setup.", width_inches=5.6)
    ...
    doc.save(synopsis_docx_path)
    print(f"Successfully updated Synopsis Document with images at: {synopsis_docx_path}")
...
if __name__ == "__main__":
    ...
    master_docx = os.path.join(docs_dir, "AgriSense_Final_Report.docx")
    master_md = os.path.join(docs_dir, "AgriSense_Final_Report.md")
    ...
    # 1. Copy images
    drone_img, ground_img = copy_images_to_docs(docs_dir, brain_dir)

    # 2. Update Master Report Docx
    update_master_report_with_images(master_docx, drone_img, ground_img)

    # 3. Export Master Report MD
    ...
    with open(master_md, "w", encoding="utf-8") as f:
        f.write("\n".join(md_lines))
    print(f"Successfully exported Master Report MD at: {master_md}")

    # 4. Update Synopsis Docx
    update_synopsis_with_images(synopsis_docx, drone_img, ground_img)

    # 5. Export Synopsis MD
    doc_syn = Document(synopsis_docx)
    syn_md_lines = []
    for p in doc_syn.paragraphs:
        t = p.text.strip()
        if not t:
            continue
        if t.startswith("PROJECT SYNOPSIS"):
            syn_md_lines.append(f"# {t}\n")
        elif t.startswith("1.") or t.startswith("2.") or t.startswith("3.") or t.startswith("4.") or t.startswith("5.") or t.startswith("6.") or t.startswith("7.") or t.startswith("8.") or t.startswith("9."):
            syn_md_lines.append(f"\n## {t}\n")
        elif "Figure " in t:
            if "Figure 5.1" in t:
                syn_md_lines.append(f"\n![Aerial Drone Payload](agrisense_drone_payload.jpg)\n*{t}*\n")
