...
def insert_budget_section(docx_path):
    print(f"Modifying {docx_path} IN-PLACE to add Section 7 Budget & Cost Estimation...")
    doc = Document(docx_path)
    
    # Locate Chapter 5 heading '5. CONCLUSION & FUTURE SCOPE'
    target_idx = None
    for i, p in enumerate(doc.paragraphs):
        t = p.text.strip()
        if "5. CONCLUSION" in t or "CONCLUSION & FUTURE SCOPE" in t:
            target_idx = i
            break

    if target_idx is None:
        print("Error: Could not find Chapter 5 heading!")
        return
    ...
    # Create table element and insert into DOM before target_p._element
    ...
    doc.save(docx_path)
    print(f"Successfully added Section 7 to {docx_path}!")
...
if __name__ == "__main__":
    ...
    insert_budget_section(onedrive_path)
    insert_budget_section(scratch_path)
