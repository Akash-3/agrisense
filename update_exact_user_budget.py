...
def generate_synopsis_new_format(docx_path, md_path):
    print(f"Building AgriSense Project Synopsis with exact user budget at {docx_path}...")
    doc = Document()
    ...
    doc.save(docx_path)
    print(f"Successfully updated AgriSense Project Synopsis DOCX at: {docx_path}")
    ...
    md_lines.append("\n### Project Cost Estimation Table (Exact User Specification)\n")
    md_lines.append("| Component Category | Items Included | Budget Profile |")
    md_lines.append("|---|---|---|")
    for cat, items, budg in cost_data_user:
...
def update_master_report_budget(docx_path):
    print(f"Updating Budget Table in Master Report {docx_path}...")
    doc = Document(docx_path)
    
    for tbl in doc.tables:
        if len(tbl.rows) > 0 and len(tbl.rows[0].cells) > 0:
            if "Component Category" in tbl.rows[0].cells[0].text:
                # Rebuild rows with user exact budget
                # We retain header row 0, update rows 1 to 5
                new_rows = [
                    ("Microcontrollers", "Raspi Zero W & ESP32 DevKit V1 Boards (Drone & Ground)", "₹2500"),
                    ("Optical Sensors", "Adafruit AS7341 10-Channel Breakout Board", "₹2500"),
                    ("Environmental Sensors", "Capacitive Soil Probe v1.2 + DHT22 + MQ-2", "₹1500"),
                ...
                # If table has fewer than 6 rows, resize or re-fill
                for r_i, (cat, items, budg) in enumerate(new_rows, start=1):
                    if r_i < len(tbl.rows):
                        ...
                        for c_idx in range(3):
                            ...
                            if p.runs:
                                ...
                                if r_i == 5 or c_idx == 0:
                                    p.runs[0].font.bold = True
                            fill_bg = "1E3A8A" if r_i == 5 else ("F8FAFC" if r_i % 2 == 0 else "FFFFFF")
                            if r_i == 5:
                                p.runs[0].font.color.rgb = RGBColor(0xFF, 0xFF, 0xFF)
                                set_cell_background(cells[c_idx], "1E3A8A")
                            else:
                                set_cell_background(cells[c_idx], fill_bg)

    doc.save(docx_path)
    print(f"Successfully updated Master Report Budget Table at: {docx_path}")
...
if __name__ == "__main__":
    ...
    generate_synopsis_new_format(syn_docx_path, syn_md_path)
    
    # Copy to OneDrive
    onedrive_syn = r"C:\Users\tempm\OneDrive\Documents\AgriSense_Project_Synopsis.docx"
    shutil.copy(syn_docx_path, onedrive_syn)
    print(f"Copied updated synopsis to: {onedrive_syn}")

    # Also update Master Report documents
    master_docx1 = r"C:\Users\tempm\OneDrive\Documents\AgriSense Report.docx"
    master_docx2 = os.path.join(docs_dir, "AgriSense_Final_Report.docx")
    update_master_report_budget(master_docx1)
    update_master_report_budget(master_docx2)
