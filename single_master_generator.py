...
def clean_strictly_all_except_master(docs_dir, master_base="AgriSense_Final_Report"):
    print("Purging ALL secondary files from docs directory...")
    all_files = glob.glob(os.path.join(docs_dir, "*"))
    for f in all_files:
        basename = os.path.basename(f)
        if basename.startswith("~$"):
            continue
        if basename != master_base + ".docx" and basename != master_base + ".md":
            try:
                os.remove(f)
...
def build_single_master_report(output_docx_path):
    doc = Document()
    ...
    # Save ONLY to the single master path output_docx_path!
...
if __name__ == "__main__":
    ...
    # Step 2: Build docx
    try:
        build_single_master_report(master_docx_path)
