...
def generate_synopsis_new_format(docx_path, md_path):
    print(f"Building AgriSense Project Synopsis with Cost Estimation at {docx_path}...")
    doc = Document()
    
    # Set page margins: 1 inch top/bottom/left/right
    ...
    set_cell_background(cell_b, "1E3A8A") # Dark Blue Shading
    ...
    p_b4 = cell_b.add_paragraph()
    p_b4.alignment = WD_ALIGN_PARAGRAPH.RIGHT
    p_b4.paragraph_format.space_after = Pt(14)
    r4 = p_b4.add_run("Department of Information Technology\nB.Sc. IT Semester-V Candidates | Academic Year 2025–2026   ")
    r4.font.name = 'Arial'
    r4.font.size = Pt(10)
    r4.font.italic = True
    r4.font.color.rgb = RGBColor(0xDB, 0xEA, 0xFE)
    ...
    add_bullet("To design and assemble", "a feather-light 82g aerial optical sensor payload featuring an ESP32 DevKit V1 [1], Adafruit AS7341 10-channel multi-spectral sensor [2], DHT22 microclimate sensor, and MQ-2 smoke detector.")
    ...
    add_bullet("To ensure complete economic accessibility", "by keeping total hardware costs under ₹6,000 and utilizing 100% free open-source software.")

    # ==================== 5. PROPOSED MODULES ====================
    add_sec_heading("5. Proposed Modules")
    ...
    tech_data = [
        ("Microcontroller Board", "ESP32 DevKit V1", "32-bit dual-core 240MHz chip with built-in Wi-Fi, low power, 400kHz I2C bus"),
    ...
    # ADDED ITEMIZATION COST ESTIMATION SECTION
    add_sub_heading("Project Cost Estimation & Hardware Budget Profile (INR ₹)")
    add_p("Commercial precision agriculture survey drones cost over ₹50,000 to ₹1,00,000, placing them completely out of reach for small family farms and educational institutions. AgriSense achieves 100x cost reduction by pairing low-cost consumer electronics with 100% free open-source software:")

    table_cost = doc.add_table(rows=7, cols=3)
    table_cost.alignment = WD_TABLE_ALIGNMENT.CENTER
    
    headers_cost = ["Component Category", "Items Included", "Budget Profile (INR ₹)"]
    ...
    cost_data = [
        ("Microcontrollers", "2x ESP32 DevKit V1 Boards (Drone & Ground)", "₹1,000  (₹500 each)"),
        ("Optical Sensors", "Adafruit AS7341 10-Channel Breakout Board", "₹2,500"),
        ("Environmental Sensors", "Capacitive Soil Probe v1.2 + DHT22 + MQ-2", "₹730  (₹250 + ₹300 + ₹180)"),
        ("Drone Body & Power", "Makeshift Quadcopter Frame & LiPo Battery", "₹1,700"),
        ("Software Stack", "FastAPI, React.js, PyTorch, Chart.js, PostgreSQL", "100% FREE Open Source"),
        ("Total Estimated Cost", "Complete Dual-Node System Deployment", "₹5,930  (Total Hardware Cost)")
    ]

    for r_idx, (cat, items, budg) in enumerate(cost_data, start=1):
        ...
        for c_idx in range(3):
            ...
            if cell_p.runs:
                ...
                if r_idx == 6 or c_idx == 0:
                    cell_p.runs[0].font.bold = True
            fill_bg = "1E3A8A" if r_idx == 6 else ("F8FAFC" if r_idx % 2 == 0 else "FFFFFF")
            if r_idx == 6:
    ...
    add_p("\nFinancial Summary: Total hardware cost is ₹5,930 (over 100x cheaper than commercial farm drones), while software licensing costs are zero. This makes precision farming accessible to smallholder farmers worldwide!")
    ...
    add_bullet("100x Cheaper Than Commercial Survey Drones —", "replaces $5,000–$10,000 industrial hyperspectral drones with an accessible ₹5,930 consumer electronics hardware setup.")
    ...
    doc.save(docx_path)
    print(f"Successfully saved AgriSense Project Synopsis DOCX at: {docx_path}")
    ...
    md_lines.append("**Department of Information Technology | B.Sc. IT Semester-V**\n")

    for p in doc_read.paragraphs:
        t = p.text.strip()
        if not t or "AgriSense" == t or "A Low-Cost, Dual-Node" in t or "Project Synopsis |" in t or "B.Sc. IT Semester-V" in t:
            continue
    ...
    md_lines.append("| # | Module Name | What it does |")
    md_lines.append("|---|---|---|")
    for num, name, desc in modules_data:
        md_lines.append(f"| {num} | **{name}** | {desc} |")

    md_lines.append("\n### Technology Stack Table\n")
    md_lines.append("| Category | Technology | Why we chose it |")
    md_lines.append("|---|---|---|")
    for cat, tech, why in tech_data:
        md_lines.append(f"| **{cat}** | {tech} | {why} |")

    md_lines.append("\n### Project Cost Estimation Table (INR ₹)\n")
    md_lines.append("| Component Category | Items Included | Budget Profile (INR ₹) |")
    md_lines.append("|---|---|---|")
    for cat, items, budg in cost_data:
        md_lines.append(f"| **{cat}** | {items} | **{budg}** |")

    with open(md_path, "w", encoding="utf-8") as f:
        f.write("\n".join(md_lines))
    print(f"Successfully exported AgriSense Project Synopsis MD at: {md_path}")

if __name__ == "__main__":
    docs_dir = r"C:\Users\tempm\.gemini\antigravity\scratch\agrisense\docs"
    docx_path = os.path.join(docs_dir, "AgriSense_Project_Synopsis.docx")
    md_path = os.path.join(docs_dir, "AgriSense_Project_Synopsis.md")
    
    generate_synopsis_new_format(docx_path, md_path)
